  ...
  public:
    ...
    public:
      ...
      Iterator( const Node_handle root, const Segment_3& s) {
        ...
        candidates.push_back( Candidate( root, s));
        ++(*this); // place the iterator in the first intersected cell
      }
      Iterator( const Self& i) : candidates(i.candidates), node(i.node) {}
      Self& operator++() {
  ...
  while( !candidates.empty()) {
    ...
    if( n->is_leaf()) {
      ...
      if( src_side == tgt_side)
        candidates.push_back( Candidate( get_child_by_side( n, src_side), s));
      else {
        Segment_3 s1, s2;
        divide_segment_by_plane( s, n->plane(), s1, s2);
        candidates.push_back( Candidate( get_child_by_side( n, tgt_side), s2)); // cell on target pushed first
        candidates.push_back( Candidate( get_child_by_side( n, src_side), s1));
      }
    }
  }
}
    ...
    private:
      ...
      void divide_segment_by_plane(const Segment_3& s, const Plane_3& pl,
                                    Segment_3& s1, Segment_3& s2) {

        const auto result = traits.intersect_object()(pl, s);
        const Point_3* ip = (result) ? std::get_if<Point_3>(&*result) : nullptr;
        CGAL_assertion(ip != nullptr);

        Point_3 p = normalized(*ip);
        s1 = Segment_3(s.source(), p);
        s2 = Segment_3(p, s.target());
...
static Point_3 find_median_point(Vertex_list& V, int coord) {
    ...
    switch(coord) {
        case 0: return find_median_point<Smaller_x_3>(V);
        case 1: return find_median_point<Smaller_y_3>(V);
        case 2: return find_median_point<Smaller_z_3>(V);
        default:
            CGAL_error_msg("Invalid coordinate index");
...
Segment_3 ray_to_segment(const Ray_3& r) const
{
  ...
  const auto result = traits.intersect_object()( pl_on_minus_x, r);
  const Point_3* ip = (result) ? std::get_if<Point_3>(&*result) : nullptr;
  if( ip == nullptr || pl_on_minus_x.has_on(p)) {
    q = r.source() + vec;
  } else {
    q = normalized(*ip);
  }
